...
def create_modules(modules_defs: list, img_size):
    ...
    routs = []  # list of layers which rout to deeper layers 路由到更深的层列表
    yolo_index = -1
    ...
    for i, mdef in enumerate(modules_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            modules.add_module(
                'deconvolutional',
                nn.ConvTranspose2d(in_channels=output_filters[-1],
                                   out_channels=filters,
            ...
            if ONNX_EXPORT:  # explicitly state size, avoid scale_factor
                ...
                modules = nn.Upsample(scale_factor=mdef["stride"],mode='bilinear')
                # stride = 2时二倍向上采样，常常出现在U-Net网络架构里；

        elif mdef["type"] == "route":  # [-2],  [-1,-3,-5,-6], [-1, 61]  三种形式 返回前2层、将几个concat、两个concat
            layers = mdef["layers"]
...
class Darknet(nn.Module):
    ...
    """
    def __init__(self, cfg, img_size=(448, 448), verbose=True):
...
def get_yolo_layers(self):
    ...
    :return:
    """
    return [i for i, m in enumerate(self.module_list) if m.__class__.__name__ == 'YOLOLayer']  # [89, 101, 113]   [92,103,114]
