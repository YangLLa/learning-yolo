...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")  # 度量记录器，是一个在训练过程中输出的类
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'Epoch: [{}]'.format(epoch)

    lr_scheduler = None
    """
    在训练初期，网络参数是随机初始化的， loss 很大，导致 gradient 很大，此时希望 learning rate 小一点，防止梯度爆炸；
    而在训练中期，我们希望 learning rate 大一点，加速网络训练，在训练末期我们希望网络收敛到 local minima，此时 learning rate 也应该小一点。
    warm up 的一套流程正好迎合了这种需求。
    训练初始，warm up 把学习率设置得很小，随着训练的进行，学习率逐渐上升，最终达到正常训练的学习率。这个阶段就是 warm up 的核心阶段。
    接着，随着训练的进行，我们希望逐渐减小学习率，（learning rate decay），训练完成时，learning rate 降为 0。

    """
    if epoch == 0 and warmup is True:  # 当训练第一轮（epoch=0）时，启用warmup训练方式，可理解为热身训练
        # 通过操作训练初始阶段的lr使得模型参数更快的收敛 并且达到更高的精度
    ...
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # Multi-Scale  当打开多尺度训练时，cfg文件中设置的width和height就不管用了，width会在320-608之间随机取值，每10轮随机改变一次
        if multi_scale:
        ...
        with amp.autocast(enabled=scaler is not None):
            ...
            # loss  计算损失 需要传入的三个信息（预测值，目标值，模型）
            loss_dict = compute_loss(pred, targets, model)
            losses = sum(loss for loss in loss_dict.values())   # 三个损失相加
